...
class LatentStyleTrainer:
    """
    Класс для дообучения генератора стиля с использованием направленной функции потерь CLIP.
    """
    def __init__(
        self,
        generator,
        ...
        # Сохранение параметров и инициализация генераторов
        self.device = device
        ...
        # Создание копий генератора — одна замороженная, одна обучаемая
        self.model = {
            "generator_frozen": copy.deepcopy(generator).to(device).eval(),
            "generator_train": copy.deepcopy(generator).to(device).train()
        }

        # Логарифмы весов для потерь CLIP и L2 (динамическая подстройка)
        self.lambda_t = torch.tensor(
            [math.log(lambda_clip_init) + 1e-6, math.log(lambda_l2_init) + 1e-6],
            device=device,
            requires_grad=True
        )

        # Оптимизатор для весов потерь
        self.optimizer_lambda = torch.optim.Adam([self.lambda_t], lr=lr_lambda) 
        # Оптимизатор для обучаемого генератора
    ...
    def sample_latent_w(self, seed=None):
        """
        Генерация латентного вектора W из случайного Z.
        """
        if seed is not None:
    ...
    def train(self, epochs, freeze_each_epoch=True, reclassify=False, seed=None):
        """
        Основной цикл обучения генератора.
        """
        ...
        for epoch in range(1,epochs+1):
            ...
            # Переопределение источника текста посредством классификации
            if reclassify:
    ...
    def visualize_images(self, generated_img_frozen, generated_img_style, epoch):
        """
        Визуализация сгенерированных изображений до и после стилизации.
        """
    ...
    def visualize_clip_directions(self, image_frozen, image_styled, text_target, text_source, preprocess):
        """
        Визуализация направлений эмбеддингов CLIP.
        """    
