...
def freeze_layers_adaptive(model_train, model_frozen, text_target_features, k=5, auto_layer_iters=3, device='cuda'):
    """
    Выбор top-k наиболее "активных" слоев в модели StyleGAN,
    которые изменяются сильнее всего при попытке привести изображение к текстовому описанию (через CLIP).
    Все остальные слои замораживаются.
    """
    batch_size_temp = 2
    latent_dim = model_frozen.style_dim
    
    # Генерируем случайные латентные векторы z и преобразуем их в W-пространство
    ...
    # Извлекаем потенциальные слои генератора для разморозки
    ...
    # Модули, которые будут разморожены
    used_modules = [potential_layers[idx] for idx in used_layers]
    
    # Имена слоев для разморозки
    ...
    # Размораживаем только выбранные слои
...
def freeze_layers(model_train, freeze_mapping=True, freeze_initial_blocks=2, freeze_torgb=True):
    """
    Упрощенная версия заморозки слоев StyleGAN — может замораживать сеть маппинга (mapping network), 
    начальные блоки и слои toRGB.
    """  
    for name, param in model_train.named_parameters():
        
        # По умолчанию размораживаем все параметры
        param.requires_grad = True
        
        # Замораживаем сеть маппинга
        if freeze_mapping and name.startswith("style."):
            param.requires_grad = False
            
        # Замораживаем начальные блоки
        if freeze_initial_blocks >= 1:
            if name.startswith("conv1.") or name.startswith("to_rgb1."):
                param.requires_grad = False
                
        # Замораживаем последующие блоки в зависимости от параметра
        for i in range(freeze_initial_blocks - 1):
            if name.startswith(f"convs.{i}.") or name.startswith(f"to_rgbs.{i}."):
                param.requires_grad = False
                
        # Замораживаем все слои toRGB
        if freeze_torgb:
...
def freeze_layers_adaptive_fine_grained(model_train, model_frozen, text, freeze_conv_weights=True, freeze_to_rgb_weights=True, k = 5, device='cuda'):
    """
    Более точная версия adaptive_layer_freezing — позволяет раздельно контролировать 
    заморозку весов сверток (conv) и слоев toRGB.
    """
    batch_size = 2
    latent_dim = 512
    ...
    # Определение важных слоев с помощью оптимизации
    ...
    # Составляем список потенциальных слоев
    all_children = list(model_train.children())
    potential_layers = [
        all_children[2], 
        all_children[3]   
    ...
    # Разморозка параметров в зависимости от типа слоя
    for name, param in model_train.named_parameters():
        if any(name.startswith(used_name) for used_name in used_layer_names):
...
def freeze_layers_adaptive_fine_tune(model_train, model_frozen, text, epochs=3, k=6, device='cuda'):
    """    
    Еще один способ выбора слоев — отслеживание параметров, которые 
    изменяются сильнее всего на протяжении нескольких эпох.
    """
    # Создаем копию модели для наблюдения за изменениями параметров
    generator_observation = copy.deepcopy(model_train)
    generator_observation.to(device)
    generator_observation.train()

    # Определяем потенциальные слои
    ...
    # Обучаем копию модели и записываем изменения нормы параметров 
    ...
    # Категоризируем параметры по типам
    categories = {
        'conv.weight': [],
