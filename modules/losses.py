...
class CLIPLoss(torch.nn.Module):
    """
    Классическая функция потерь CLIP для измерения сходства между изображением и текстом.
    """
    ...
    def forward(self, image, text_features_normalized):
        """
        Вычисление потерь CLIP между изображением и нормализованным текстовым эмбеддингом.
        """
        image = self.avg_pool(self.upsample(image))
        image_vector = self.model.encode_image(image)
        image_vector = image_vector / image_vector.norm(dim=-1, keepdim=True)
        # Вычисляем косинусное сходство и преобразуем в loss
...
class CLIPDirectionalLoss(torch.nn.Module):
    """
    Направленная функция потерь CLIP (Directional CLIP Loss).
    Измеряет направление изменения признаков от исходного изображения к целевому в пространстве CLIP.
    """    
    def __init__(self):
    ...
    def forward(self, img_frozen, img_styled, text_features_source_norm, text_features_target_norm):
        """
        Аргументы:
            img_frozen (torch.Tensor): изображения из замороженного генератора
            img_styled (torch.Tensor): изображения из стилизованного генератора
            text_features_source_norm (torch.Tensor): нормализованные текстовые эмбеддинги исходного стиля
            text_features_target_norm (torch.Tensor): нормализованные текстовые эмбеддинги целевого стиля
        """
        ...
        # Предобработка изображений для CLIP
        ...
        # Извлечение признаков CLIP без градиентов
        with torch.no_grad():
